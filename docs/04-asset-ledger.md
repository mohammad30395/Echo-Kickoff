...
| VIS-016 | `assets/ui/echo_theme.tres`, `scripts/entities/listener_visual.gd`, `scripts/interactions/extraction_gate_visual.gd` | Runtime UI theme and procedural animation/presentation polish | Jam-authored Godot Theme resource and typed GDScript | 2026-07-20 | Original work; Godot StyleBoxFlat, CanvasItem lines, circles, polygons, and arcs only | Distinct normal/hover/pressed/focus/disabled buttons; animated Listener limbs, breathing, gait, hearing spines, alert and attack feedback; compact dual-panel extraction gate | Approved |
