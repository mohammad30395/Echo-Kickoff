...
func _test_release_balance_values() -> void:
	...
	_expect(is_equal_approx(listeners[0].investigate_speed, 110.0), "Primary Listener investigate speed is not campaign tuned.")
	_expect(is_equal_approx(listeners[0].chase_speed, 150.0), "Primary Listener chase speed is not campaign tuned.")
	_expect(is_equal_approx(listeners[0].hearing_sensitivity, 1.0), "Primary Listener hearing is not release tuned.")
	_expect(is_equal_approx(listeners[0].search_duration, 3.2), "Primary Listener search duration is not campaign tuned.")
	_expect(is_equal_approx(listeners[1].investigate_speed, 110.0), "South Listener investigate speed is not campaign tuned.")
	_expect(is_equal_approx(listeners[1].chase_speed, 150.0), "South Listener chase speed is not campaign tuned.")
	_expect(is_equal_approx(listeners[1].hearing_sensitivity, 1.0), "South Listener hearing is not campaign tuned.")
